...
const CreateQuiz = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        const res = await fetch("http://localhost:5000/api/modules/add-quiz", {
            method: "POST",
    ...
    return (
        <div className="max-w-lg mx-auto bg-white shadow-lg rounded-lg p-6 mt-10">
            <h2 className="text-2xl font-bold mb-4 text-center text-gray-700">Create New Quiz</h2>
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                {questions.map((q, qIndex) => (
                    <div key={qIndex} className="border p-4 rounded-md">
                        <input type="text" value={q.question} onChange={(e) => handleQuestionChange(qIndex, "question", e.target.value)} placeholder="Question" required
                        ...
                        <input type="text" value={q.answer} onChange={(e) => handleQuestionChange(qIndex, "answer", e.target.value)} placeholder="Correct Answer" required
                            className="w-full px-3 py-2 border rounded-md focus:ring-2 focus:ring-blue-400 mt-2"/>
                    </div>
                ))}

                <button type="button" onClick={addQuestion} className="bg-gray-600 text-white py-2 px-4 rounded-md">
                    ➕ Add Question
                </button>
