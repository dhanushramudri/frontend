    ...
    return (
          <div className="container">
            <div className="sidebar">
              <h3><img className='logoimg' src='billbologo.jpg' alt="Logo" /></h3>
              {isLoggedIn ? (
                <button className='btn'>
                  <Link to='/login'>Login</Link> {/* Update to 'Logout' when logged in */}
                </button>
              ) : (
                <button className='btn'>
                    ...
                    {notes.map((note) => (
                        <li className="cardadmin" key={note._id}>
                            ...
                            <div className="button-group">
                                <button className="admiedit" onClick={() => handleEdit(note)}>Edit</button>
                                <button className="admidelete" onClick={() => handleDelete(note._id)}>Delete</button>
                            </div>
                        </li>
                    ))}
                </ul>
